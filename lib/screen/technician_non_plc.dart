...
class TechnicianNonPlc extends StatefulWidget {
  const TechnicianNonPlc({Key? key}) : super(key: key);

  @override
  State<TechnicianNonPlc> createState() => _TechnicianNonPlcState();
}

class _TechnicianNonPlcState extends State<TechnicianNonPlc>
  ...
  void onOpenRequestSuccess(FaultListResponse data) {
    if (data.status == 200) {
      ...
      for (int i = 0; i < resultList.length; i++) {
         ...
         * */
        if ((resultList[i].isPlc == 0 &&
                resultList[i].acceptedStatus == 1 &&
                resultList[i].progressStatus != 3) ||
            (resultList[i].isPlc == 0 && resultList[i].progressStatus == 2)) {
